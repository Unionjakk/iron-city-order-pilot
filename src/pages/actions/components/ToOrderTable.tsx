...
import ToOrderOrderComponent from "./ToOrderOrderComponent";
...
const ToOrderTable = ({ orders, refreshData }: ToOrderTableProps) => {
  return (
        ...
        <TableBody>
          {orders.map((order) => (
            <React.Fragment key={`order-group-${order.id}`}>
              <ToOrderOrderComponent order={order} refreshData={refreshData} />
